...
import { isFullGenerationAction, isLightEditAction } from "../shared/ai/ai-limits.ts";
import type { AiActionType } from "../shared/ai/ai-types.ts";
import { DEFAULT_AI_FEATURE_FLAGS } from "../shared/ai/ai-feature-flags.ts";
import { type ProductPlanTier } from "../shared/plans/plan-tiers.ts";
import { resolvePlanTier } from "../shared/plans/feature-access.ts";
...
export async function getCloudData(userId: string) {
  const result = await dbPool.query(
    "SELECT planner_json as \"plannerData\", budget_json as \"budgetData\", meal_json as \"mealData\", cashflow_json as \"cashflowData\", habits_json as \"habitsData\" FROM app_user_data WHERE user_id = $1 LIMIT 1",
    [userId],
  );

  if (!result.rowCount) {
    await dbPool.query(
      "INSERT INTO app_user_data (user_id, planner_json, budget_json, meal_json, cashflow_json, habits_json) VALUES ($1, '{}'::jsonb, '{}'::jsonb, '{}'::jsonb, '{}'::jsonb, '{}'::jsonb)",
      [userId],
    );
    return { plannerData: null, budgetData: null, mealData: null, cashflowData: null, habitsData: null };
  }

  return result.rows[0] as { plannerData: unknown; budgetData: unknown; mealData: unknown; cashflowData: unknown; habitsData: unknown };
}

export async function saveCloudData(userId: string, payload: CloudDataPayload) {
  await dbPool.query(
    `
    INSERT INTO app_user_data (user_id, planner_json, budget_json, meal_json, cashflow_json, habits_json, updated_at)
    VALUES ($1, COALESCE($2::jsonb, '{}'::jsonb), COALESCE($3::jsonb, '{}'::jsonb), COALESCE($4::jsonb, '{}'::jsonb), COALESCE($5::jsonb, '{}'::jsonb), COALESCE($6::jsonb, '{}'::jsonb), NOW())
    ON CONFLICT (user_id)
    DO UPDATE SET
      ...
      habits_json = COALESCE($6::jsonb, app_user_data.habits_json),
      updated_at = NOW();
    `,
    [
      userId,
      ...
      payload.habitsData ? JSON.stringify(payload.habitsData) : null,
    ],
  );
}
