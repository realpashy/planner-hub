...
function MacroPill({ value }: { value: string }) {
  return (
    <span className="inline-flex items-center rounded-[5px] border border-border/65 bg-background/80 px-2 py-1 text-[10px] font-semibold text-foreground shadow-[var(--app-shadow)]">
      {value}
    </span>
  );
}
...
export function PlannerDayCard({ day, selected = false, onOpen }: PlannerDayCardProps) {
  return (
    <motion.button
      ...
      dir="rtl"
      className={cn(
        "w-full rounded-[calc(var(--radius)+0.9rem)] border p-3.5 text-right shadow-xl transition-all",
        selected
    ...
    >
      <div className="space-y-3">
        <div className="meal-leading-row gap-2">
        ...
        <div className="space-y-1 rounded-[5px] border border-border/70 bg-background/65 p-2.5 shadow-[var(--app-shadow)]">
          {day.meals.map((meal, index) => {
            ...
            return (
              <div
                key={meal.id}
                className={cn(
                  "meal-preview-row rounded-[calc(var(--radius)+0.25rem)] px-2 py-1.5 text-right",
                  index !== day.meals.length - 1 && "border-b border-dashed border-slate-200/70 dark:border-slate-700/60",
                )}
              >
                <div className="meal-preview-leading">
                  <span
                    className={cn(
        ...
        <div className="meal-leading-row rounded-[5px] border border-primary/15 bg-primary/[0.06] px-3 py-2.5 text-right">
